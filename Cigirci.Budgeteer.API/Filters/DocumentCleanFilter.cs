...
public class DocumentCleanFilter : IDocumentFilter
{
    ...
    private OpenApiDocument? _swaggerDoc;
    ...
    private bool RemoveUnused(OpenApiDocument swaggerDoc, bool includeAllSchemas)
    {
        ...
        if (includeAllSchemas)
            ...
            AddSchemaCollectionReferenceCount(
                swaggerDoc.Components.Schemas.Where(x => countByDefinitionReference[SchemaV3Prefix + x.Key] != 0)
                    .Select(x => x.Value), !includeAllSchemas, 0);
        ...
        foreach (var countByRef in countByDefinitionReference.Where(x => x.Value == 0))
        {
            ...
            if (swaggerDoc.Components.Schemas.Remove(definitionKey)) anyDefinitionRemoved = true;
        }
        ...
        foreach (var countByRef in countByDefinitionReference.Where(x => x.Value == 1))
        {
            ...
            if (swaggerDoc.Components.Schemas.TryGetValue(definitionKey, out var openApiSchema)
    ...
    private void AddSchemaCollectionReferenceCount(IEnumerable<OpenApiSchema> definitions, bool alsoCountSubSchemas,
        int subSchemaDepth)
    {
    ...
    private void AddOperationReferenceCount(IEnumerable<OpenApiOperation> operations, bool alsoCountSubSchemas,
        int subSchemaDepth)
    {
    ...
    private void AddRequestReferenceCount(OpenApiRequestBody requestBody, bool alsoCountSubSchemas, int subSchemaDepth)
    {
        ...
        if (requestBody.Reference != null && !string.IsNullOrWhiteSpace(requestBody.Reference.ReferenceV3))
        {
            ...
            if (alsoCountSubSchemas && _swaggerDoc is not null &&
                _swaggerDoc.Components.Schemas.TryGetValue(requestBody.Reference.Id, out var subSchema))
    ...
    private void AddResponseReferenceCount(IDictionary<string, OpenApiResponse> responsesByHttpStatus,
        bool alsoCountSubSchemas, int subSchemaDepth)
    {
        if (responsesByHttpStatus == null) return;
    ...
    private void AddParameterReferenceCount(IList<OpenApiParameter> parameters, bool alsoCountSubSchemas,
        int subSchemaDepth)
    {
        if (parameters == null) return;

        foreach (var param in parameters) AddSchemaReferenceCount(param.Schema, alsoCountSubSchemas, subSchemaDepth);
    }
    ...
    private void AddSchemaReferenceCount(OpenApiSchema schema, bool alsoCountSubSchemas, int subSchemaDepth)
    {
        ...
        if (alsoCountSubSchemas && subSchemaDepth > 5) alsoCountSubSchemas = false;
        ...
        if (schema.Reference != null && !string.IsNullOrWhiteSpace(schema.Reference.ReferenceV3))
        {
            ...
            if (alsoCountSubSchemas && _swaggerDoc is not null &&
                _swaggerDoc.Components.Schemas.TryGetValue(schema.Reference.Id, out var subSchema))
                AddSchemaReferenceCount(subSchema, alsoCountSubSchemas, subSchemaDepth + 1);
        }

        if (schema.Items != null && schema.Items.Reference != null &&
            !string.IsNullOrWhiteSpace(schema.Items.Reference.ReferenceV3))
        {
            countByDefinitionReference[schema.Items.Reference.ReferenceV3]++;
            if (alsoCountSubSchemas && _swaggerDoc is not null &&
                _swaggerDoc.Components.Schemas.TryGetValue(schema.Items.Reference.Id, out var subSchema))
                AddSchemaReferenceCount(subSchema, alsoCountSubSchemas, subSchemaDepth + 1);
        }
    ...
    private int CountReferencesToSpecificSchemaInSchema(OpenApiSchema schema, string schemaName)
    {
        ...
        if (schema.Reference != null && schema.Reference.ReferenceV3 == schemaName) count++;

        if (schema.Items != null && schema.Items.Reference != null &&
            schema.Items.Reference.ReferenceV3 == schemaName) count++;
